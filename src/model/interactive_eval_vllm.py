...
def load_pep_benchmark(folder: str) -> list[tuple[str, dict]]:
...
async def eval_benchmark(benchmark, model_name: str, peps: list[str], pep_idiom_specs: dict, zero_shot: bool=False):
    ...
    for blob_id, code_file, gt in tqdm(benchmark):
        ...
        try: cot, report = parse_response(response["model_response"])
            ...
            cot = ""
            report = response["model_response"]
        predicitons = load_linter_results(report)
        resp_and_gt.append({
            "model_response": predicitons,
            "ground_truth": gt,
        })

        print(f"predicted {blob_id}: {[p['code'] for p in predicitons]}")
        # print("GT:", gt)
...
if __name__ == "__main__":
    ...
    if args.offline:
        pep_benchmark = load_pep_benchmark("./data/pep_benchmark")
        asyncio.run(eval_benchmark(pep_benchmark, model_name=model_name, peps=pep_list, pep_idiom_specs=pep_idiom_specs, zero_shot=args.zero_shot))
